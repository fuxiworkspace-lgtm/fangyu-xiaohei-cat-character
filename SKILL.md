...
description: |
  生成“小黑 + 真实物件 + 物理动作 + 留白叙事”的中文配图，并可根据输入内容的语言场景、情绪状态和核心动作路由房语色彩小黑（小黑/小红/小绿/小蓝/小黄）。默认输出 16:9 正文配图，用于互联网打工人共鸣图、AI 时代职场焦虑、程序员/产品/创作者处境和正文观点隐喻图；遇到“彩蛋模式 / 长卷故事图 / 超横版 / 个人经历 / 项目复盘 / 产品演化 / 成长路径”时，输出小黑长卷故事图。标准模式默认 #FFFFFF 纯白背景；彩蛋长卷模式使用高级近白背景和一条真实物件人生线。样例是高质量模板母版和出图质量标尺；必须对齐其比例、留白、动作清晰度和叙事关系，但不能复刻其物件组合、空间拓扑、小黑姿态或标签位置。
when-to-use: |
  用户需要为中文文章、帖子、教程、案例、项目复盘或个人经历生成“小黑 + 真实物件 + 物理动作”的高质量配图时触发。
  触发词：小黑实物场景图、小黑 2.0、实物图、真实物件、小黑 + 物件、彩色小黑、房语小黑、小红小绿小蓝小黄、打工人共鸣图、正文配图、彩蛋模式、长卷故事图、超横版、个人经历、项目复盘、产品演化、成长路径。
...
本 Skill 以既有小黑 2.0 场景工作流为完整工作流母体。除“角色 IP 定义”外，原 Skill 的全部规则、流程和资源都继承，不得自行删减或改写，包括：

- 内容理解、处境提炼、事实锚定和视觉隐喻方法；
- 标准模式与彩蛋长卷模式；
- 01-06 标准母版、07 长卷母版、母版不变量与变异规则；
- 真实物件选择、核心物理动作、短中文标签和留白叙事；
- prompt 组装方式、生成参数、画幅和输出结构；
- 颜色使用逻辑、场景色彩纪律、原创性约束和全部 QA 检查。
...
房语小黑的角色家族重点是“穿着不同颜色衣服的小猫”。黑色猫身体、猫耳、眼睛和四肢是统一底盘；彩色工作服才是区分小红、小绿、小蓝、小黄以及其他角色的第一视觉信号。

每次生成前，必须先回答：

```text
这只小猫穿什么颜色的衣服？这件衣服为什么代表它在当前场景中的角色？
```

禁止把角色理解成“给黑色身体换颜色”。身体始终保持黑色，衣服颜色、衣服的简洁轮廓和它正在做的动作共同构成角色识别。
...
如果用户要求“纯手绘小黑解释图 / 白板风流程图 / 手绘方法论图”，优先用 `$fangyu-xiaohei-character`。如果用户要求“真实物件 / 实物图 / 小黑 + 物件 / 打工人共鸣 / 更有生活质感 / 小黑 2.0”，用本 Skill。
...
- `references/xiaohei-character-lock.md`：房语小黑唯一角色 IP 标准；角色外形、形体、眼睛、动作边界和禁忌以此为准。
- `references/color-characters.md`：房语小黑色彩角色系统；当用户提到彩色小黑、颜色分身、情绪/功能配色，或内容里存在风险、修复、系统、机会、涨跌等明显颜色语义时读取。
...
- `assets/examples/`：高质量模板母版和出图质量标尺。`color-character-family-cat-clothing.png` 是当前唯一的颜色家族标准锚点；`anchors/` 是不同版式的应用锚点；`07-long-scroll-story-master.png` 是长卷空间骨架。生成时必须优先对齐黑色猫耳身体、五色工作服关系、真实物件动作和留白气质；不要复刻具体物件、空间拓扑、姿态或标签位置。
...
- 哪 2-4 个短词能一秒戳中痛点？
- 哪个真实物件天然承载这个处境？
- 这张图是否需要默认小黑，还是需要小红 / 小绿 / 小蓝 / 小黄表达风险、修复、系统或机会？
...
默认仍使用哑光黑色小黑。只有当输入内容的语言场景、情绪状态或核心动作有明确语义时，才引入彩色小黑。

需要颜色判断时读取 `references/color-characters.md`，并完成：
...
- 小黑：默认主叙事、职场荒诞、项目复盘、复杂长卷主线。
- 小红：警报、冲突、商战、风险、情绪高点、中国股市红涨。
- 小绿：修复、恢复、通过、关系连接、中国股市绿跌。
- 小蓝：系统、代码、工具、数据、AI 工作流、理性操作。
- 小黄：机会、提醒、预热、启动、点亮、灵感。

标准 16:9 默认最多 1-2 个角色；只有当内容存在清楚的多方分工时才到 3 个。彩蛋长卷可以在不同节点使用不同颜色，但颜色必须跟节点语义绑定，不要机械换色。

完成标记：已经选择默认小黑或彩色角色，并能说明颜色如何帮助 3 秒读懂；所有角色仍保持同一形体、眼睛、细四肢和物理动作逻辑，只改变身体颜色。
...
- 真实摄影物件自然融合，统一光影和透视。
- 小黑参与核心物理动作。
- 角色颜色路由：默认小黑或彩色小黑的选择理由；彩色角色只改变哑光身体颜色，不改变眼睛、四肢、比例、性格和动作逻辑。
- 2-4 个短中文手写标签。
- 少量蓝 / 粉 / 黄 / 绿 / 红点缀。
- 中等场景覆盖面积，但视觉重量轻。
...
- 彩色角色可按节点语义使用：启动/预热用小黄，系统/工具用小蓝，冲突/高峰用小红，修复/连接用小绿，主线叙事用小黑。
...
彩蛋模式要保留母版的空间结构和叙事方式，但根据当前内容替换节点主题、真实物件、小黑动作、节点文案、起点和终点表达。不要复刻母版中的个人经历、品牌或平台名称，除非用户明确提供并要求保留这些事实。
...
- 场景有没有过大、过小、过满或过散。
- 小黑是否承担核心动作。
- 彩色角色是否由内容语义驱动，而不是装饰；所有颜色小黑是否仍是同一形体系统。
- 中文短标签是否准确、少、能读。
...
- 小黑必须出现，并承担核心物理动作。
- 如使用彩色小黑，颜色必须服务内容语义；所有角色必须保持同一小黑形体、白色小眼睛、细四肢和冷静认真气质，只改变身体颜色。
...
- 彩色角色数量克制：标准模式通常不超过 2 个；长卷节点配色不机械平均撒色。
- 真实物件不压画面，节点之间有空气。
- 彩色点缀只做节奏，不抢主体。
- 输出能说明“哪张最稳、哪张需要再收一轮”。
